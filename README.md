...
#### ✨ 体验与交互改进
- **运行时状态可见**：顶部状态栏实时展示连接状态点（绿/黄/红），不再需要猜测 SDK 是否在线
- **上下文透明化**：发送消息前可看到将被传入的文件列表，可按需移除
- **工具调用流完整展示**：工具执行过程（参数、状态、结果）独立卡片展示，完成后自动折叠
- **思考流与回答分离**：AI 内部推理以折叠块展示，不与最终回答混流

#### 🎨 视觉精修
- **统一焦点环**：所有按钮和交互控件补齐 `focus-visible` 键盘焦点环（2px brand 色轮廓）
- **可访问标签**：7 个图标控件补充中文 `aria-label`（屏幕阅读器友好）
- **动画一致性**：将 `transition: all` 替换为具体属性，消除不必要的噪音动画

#### 🛡️ 诊断与恢复
- **设置页诊断入口**：设置页新增"诊断与状态"区，异步展示 SDK 端口、连接状态和存储用量
- **SDK 控制按钮**：设置页提供"重启 SDK"和"重新检测"两个独立操作
- **存储配额警告**：存储用量超过 80% 时主动提示，同一会话内仅展示一次

#### 🔧 代码质量
- 新增 22 个测试文件 / 123 个自动化测试（覆盖消息渲染、思考流、工具调用、会话管理、可访问性等核心路径）
- 关键逻辑模块化，思考流与消息流路由通过单元测试护栏保障
...
- **修复流式响应文字重复问题**：修复了 AI 回复中每个字符被重复输出 2-3 次的问题
  ...
  - **效果**：修复前 "早安早安！！有什么有什么可以可以帮帮你的你的吗吗？？" → 修复后 "早安！有什么可以帮你的吗？"
